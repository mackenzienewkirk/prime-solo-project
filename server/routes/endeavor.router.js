...
router.get('/', (req, res) => {
  ...
  WHERE "user_id" = $1
  `
  pool.query(sqlQuery, [currentUserID])
    .then((dbRes) => {
      res.send(dbRes.rows);
    })
    .catch((dbErr) => {
...
router.post('/', (req, res) => {
  ...
  const newNotes = req.body.notesInput;
  const user_id = req.user.id;
  console.log('Adding new endeavor!', newEndeavor);

  const sqlText = `
  INSERT INTO "endeavor" ("title", "budget", "materials", "inspiration", "description", "end_goal", "notes","user_id")
  VALUES ($1, $2, $3, $4, $5, $6, $7, $8);
  `;

  const sqlValues = [newTitle, newBudget, newMaterials, newInspiration, newDescription, newEndGoal, newNotes, user_id];
...
router.put('/:id', (req, res) => {
  ...
  const sqlText = `
    UPDATE "endeavor"
      ...
      WHERE "id"=$7
  `;
  pool.query(sqlText, [req.body.title, req.body.budget, req.body.materials, req.body.inspiration, req.body.description, req.body.end_goal, idToUpdate])
      .then((result) => {
          res.sendStatus(200);
      })
      .catch((error) => {
